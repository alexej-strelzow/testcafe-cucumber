...
function createServerAndRunTests() {
  createTestCafe(TEST_CAFE_HOST)
      .then((tc) => {
        ...
        return runner.run({quarantineMode: true}).catch((error) => logger.error('Caught error: ', error));
      })
      .then(() => {
      ...
      .catch((error) => logger.error('Caught error: ', error));
}

/**
 * Creates a server "live"-instance of TestCafe and starts a test-runner.
 * This instance won't get destroyed after a test run.
 */
function createLiveServerAndRunTests() {
  createTestCafe(TEST_CAFE_HOST, 1337, 1338)
      .then((tc) => {
        testCafe = tc;
        let liveRunner = tc.createLiveModeRunner();

        liveRunner = liveRunner.src(`./${TEST_FILE}`).browsers(`${BROWSER}${BROWSER_FLAGS}`.trim());

        return liveRunner.run().catch((error) => logger.error('Caught error: ', error));
      })
      .then(() => testCafe.close())
      .catch((error) => logger.error('Caught error: ', error));
}
